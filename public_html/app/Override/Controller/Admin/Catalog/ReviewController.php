 ...
 */
class ReviewController extends \ControllerCatalogReview
{

    protected $error;

    /**
     * Добавлено поле about в список отзывов
     */
    protected function getList()
    {
        ...
        if (isset($this->request->get['filter_about'])) {
            $filter_about = $this->request->get['filter_about'];
        } else {
            $filter_about = null;
        }

        if (isset($this->request->get['filter_moderator'])) {
            $filter_moderator = $this->request->get['filter_moderator'];
        } else {
            $filter_moderator = null;
        }
        ...
        if (isset($this->request->get['filter_about'])) {
            $url .= '&filter_about=' . urlencode(html_entity_decode($this->request->get['filter_about'], ENT_QUOTES, 'UTF-8'));
        }

        if (isset($this->request->get['filter_moderator'])) {
            $url .= '&filter_moderator=' . urlencode(html_entity_decode($this->request->get['filter_moderator'], ENT_QUOTES, 'UTF-8'));
        }

        if (isset($this->request->get['filter_answer'])) {
            $url .= '&filter_answer=' . urlencode(html_entity_decode($this->request->get['filter_answer'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $filter_data = array(
            ...
            'filter_about' => $filter_about,
            'filter_moderator' => $filter_moderator,
        ...
        $data['column_moderator'] = $this->language->get('column_moderator');
        ...
        $data['entry_moderator'] = $this->language->get('entry_moderator');
        ...
        $data['filter_about'] = $filter_about;
        $data['filter_answer'] = $filter_answer;
        $data['filter_moderator'] = $filter_moderator;
    ...
    protected function getForm()
    {
        ...
        if (isset($this->error['moderator'])) {
            $data['error_moderator'] = $this->error['moderator'];
        } else {
            $data['error_moderator'] = '';
        }
        ...
        if (isset($this->request->post['moderator'])) {
            $data['moderator'] = $this->request->post['moderator'];
        } elseif (!empty($review_info)) {
            $data['moderator'] = $review_info['moderator'];
        } else {
            $data['moderator'] = '';
        }
        ...
        if (isset($this->request->post['company'])) {
            ...
            $data['company'] = $review_info['company'];
        } else {
            $data['company'] = '';
        }
    ...
    /**
     * Поле product_id, raiting больше не являются обязательным 
     * @return type
     */
    protected function validateForm()
    {
        ...
        if ((utf8_strlen($this->request->post['author']) > 100)) {
            $this->error['moderator'] = $this->language->get('error_moderator');
        }
        ...
        if ((utf8_strlen($this->request->post['answer']) > 2000)) {
