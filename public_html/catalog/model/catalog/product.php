...
class ModelCatalogProduct extends Model {
	...
	*/
	...
	public function getMaxRatingProduct($category_id){
		$sql="select o_r.product_id from oc_review o_r LEFT JOIN oc_product_to_category o_c ON o_r.product_id=o_c.product_id
		where o_c.category_id=282
		group by o_r.product_id
		order by max(o_r.rating) DESC
		limit 1";
		$query = $this->db->query($sql);
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *,"
			...
			(SELECT MIN(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating_min,
			(SELECT MAX(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating_max,
		...
		*/
		if ($query->num_rows) {
			...
			return array(
				...
				'rating_min'       => $query->row['rating_min'],
				'rating_max'       => $query->row['rating_max'],
	...
	public function getProducts($data = array()) {
		...
		(SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, 
		(SELECT MIN(rating) AS total FROM oc_review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating_min, 
		(SELECT MAX(rating) AS total FROM oc_review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating_max, 
	...
	public function getProductRelated($product_ids, $rand=false,$limit=0,$table,$exclude_ids='',$showonstore=false) {
		$product_data = array();
		$order_rand="";
		$limit_str="";
		if($rand){
