...
class ModelCalcCalc extends Model {
  function getProductsByCalc($calculator_id){
    ...
    where (op.calc='".$calculator_id."' or o_cat.calc='".$calculator_id."') and op.quantity>0
  ...
  function getProductsAnalog($products_arr){
    if($products_arr){
      ...
      WHERE o_r.product_id IN (".$products_str.") and o_p.quantity>0 group by o_p.product_id order by IF(o_pa.path_id=270,1,0) ASC, o_p.quantity DESC");
      $products=[];
      foreach ($query->rows as $result) {
        $products[]=$result['product_id'];
      }
