...
class ControllerExtensionFeedTurboRSS extends Controller {
	...
	if (isset($this->request->post['turbo_rss_limit'])) {
		...
		$data['turbo_rss_limit'] = 1000;
	}

	$total =  $this->model_catalog_product->getTotalProducts(array('filter_status'=>1));
	$limit = $data['turbo_rss_limit'];
	$parts = ceil($total / $limit) - 1;

	$data['data_feed'] = array();
	$data['data_feed'][0] = HTTPS_CATALOG . 'index.php?route=extension/feed/turbo_rss&part=0';
	for ($i = 1; $i <= $parts; $i++) {
	    $data['data_feed'][$i] = HTTPS_CATALOG . 'index.php?route=extension/feed/turbo_rss&part='.$i;
	}
	...
	if (isset($this->request->post['turbo_rss_image_width'])) {
	    ...
	    $data['turbo_rss_image_width'] = $this->config->get('turbo_rss_image_width') ? $this->config->get('turbo_rss_image_width') : 300;
	}

	if (isset($this->request->post['turbo_rss_image_height'])) {
	    $data['turbo_rss_image_height'] = $this->request->post['turbo_rss_image_height'];
	} else {
	    $data['turbo_rss_image_height'] = $this->config->get('turbo_rss_image_height') ? $this->config->get('turbo_rss_image_height') : 300;
	}
